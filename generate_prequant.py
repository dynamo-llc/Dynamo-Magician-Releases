...
from load_prequant import load_quantized_model, is_blackwell
...
class WanI2V_PreQuant:
    ...
    def __init__(
        ...
        # Free temporary allocations before loading the second model.
        # On Blackwell the NF4→FP16 migration roughly triples peak RAM per
        # model, so reclaiming memory between loads prevents page-file
        # exhaustion on systems with limited virtual memory / swap.
        gc.collect()

        self.high_noise_model = load_quantized_model(high_noise_dir, device="cpu")

        # Track whether Blackwell NF4→FP16 migration has been done per model.
        # Static FP16 migration is only used in "high" VRAM mode (~28 GB/model).
        # In "low" VRAM mode, NativeLinear4bit dequantizes on-the-fly (~9 GB/model).
        self._blackwell = is_blackwell()
        self._migrated = {"low_noise_model": False, "high_noise_model": False}

        logger.info("Model loading complete!")

    @staticmethod
    def _blackwell_migrate(model, device):
        """One-time NF4→FP16 migration for a single model on Blackwell GPUs.

        Only used in 'high' VRAM mode where the ~28 GB FP16 footprint fits.
        In 'low' VRAM mode the NativeLinear4bit layers stay as-is and
        dequantize on-the-fly in their forward() method (~9 GB on GPU).
        """
        logger.info("  Blackwell: migrating NF4 → static FP16 (high VRAM)...")
        native_layers = [
            (name, module)
        ...
        logger.info("  Blackwell migration complete.")
    ...
    def _prepare_model_for_timestep(self, t, boundary):
        ...
        if self.vram_mode == "high":
            ...
            try:
                if next(required_model.parameters()).device.type == "cpu":
                    # Blackwell: convert NF4→static FP16 only in high VRAM mode
                    if self._blackwell and not self._migrated[required_model_name]:
                        self._blackwell_migrate(required_model, str(self.device))
                        self._migrated[required_model_name] = True
        ...
        # In low VRAM mode on Blackwell, NativeLinear4bit stays as NF4 and
        # dequantizes on-the-fly in forward().  No static migration needed.
        # This uses ~9 GB VRAM per model instead of ~28 GB for full FP16.
    ...
    def generate(
        ...
        # Encode image — chunked to avoid allocating (F-1)*H*W*3*4 bytes on GPU.
        # encode_image_for_video() creates one 4-frame zero chunk per iteration
        # instead of a single [3, F, H, W] tensor (saves ~4.6 GB at F=961, 480p).
        img_resized = torch.nn.functional.interpolate(
            img_tensor[None].cpu(), size=(h, w), mode="bicubic"
        )[0].to(self.device)  # [3, h, w]
        y = self.vae.encode_image_for_video(img_resized, F)
        del img_resized
        torch.cuda.empty_cache()
        y = torch.concat([msk, y])

        # Offload VAE to CPU for the duration of the diffusion loop so the
        # diffusion models have the full GPU memory budget.
        if self.vae_cpu_offload:
            self.vae.model.to("cpu")
            torch.cuda.empty_cache()
            logger.info("VAE offloaded to CPU for diffusion loop")
        ...
        with torch.amp.autocast("cuda", dtype=self.param_dtype), torch.no_grad():
            ...
            for i, t in enumerate(timesteps):
                ...
                # Live step preview: decode the first latent frame every N steps.
                if (preview_callback is not None
                        and preview_every > 0
                        and (i + 1) % preview_every == 0
                        and not self.vae_cpu_offload):
                    try:
                        with torch.no_grad():
                            preview_latent = latent[:, 0:1, :, :].detach()
                            preview_video = self.vae.decode([preview_latent])
                            preview_frame = preview_video[0][:, 0, :, :]  # [3, H, W]
                            del preview_video
                            frame_np = ((preview_frame + 1) / 2 * 255).clamp(0, 255).byte()
                            frame_np = frame_np.permute(1, 2, 0).cpu().numpy()
                            pil_frame = Image.fromarray(frame_np)
                            del preview_frame, frame_np
                        preview_callback(pil_frame, i + 1)
                    except Exception as _preview_err:
                        logger.debug(f"Preview decode skipped at step {i+1}: {_preview_err}")
            ...
            # Restore VAE to GPU for decode if it was offloaded.
            if self.vae_cpu_offload:
                self.vae.model.to(self.device)
                logger.info("VAE restored to GPU for decode")
