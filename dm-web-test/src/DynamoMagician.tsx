...
export default function DynamoMagician() {
  ...
  const handleTransmute = useCallback(async () => {
    ...
    try {
      ...
      setGenStatus("AETHER-NET STATUS: CONNECTING TO SERVER…");
      ...
      pushLog(`  VRAM  : ${vramHigh ? "HIGH" : "LOW"} mode`);
      pushLog("  ENCODING INPUT IMAGE...");

      const image_b64 = await encodeBlob();
      ...
      pushLog(`  SERVER ACK  →  JOB: ${job_id.slice(0, 8).toUpperCase()}`);
      pushLog("  SSE STREAM ESTABLISHED — MONITORING...");

      const es = new EventSource(`/stream/${job_id}`);

      es.onmessage = (ev) => {
        try {
          const msg = JSON.parse(ev.data) as { type: string; message?: string; current?: number; total?: number; output?: string };
          if (msg.type === "status") {
  ...
  }, [imageSrc, imageName, imageFileSize, prompt, width, height, steps, guidance, frames, seed, vramHigh, pushLog]);
              ...
              <div className="dm-transmute-row">
                ...
                <div style={{ position: "relative" }}>
                  <button
                    ...
                    title={isGenerating ? "Generation in progress…" : "Start generation"}
                  >
                    <TransmutePortal active={isGenerating} />
                    <span className="dm-transmute-lbl">{isGenerating ? "GENERATING…" : "GENERATE WORLD MODEL"}</span>
                  </button>
