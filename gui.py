...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.vram_switch.pack(pady=(0, 8))

        # T5 CPU Offload
        self.t5_cpu_var = ctk.BooleanVar(value=True)
        self.t5_cpu_switch = ctk.CTkSwitch(self.control_frame, text="T5 CPU Offload (saves VRAM)", variable=self.t5_cpu_var)
        self.t5_cpu_switch.pack(pady=(0, 8))

        # VAE CPU Offload
        self.vae_offload_var = ctk.BooleanVar(value=False)
        self.vae_offload_switch = ctk.CTkSwitch(self.control_frame, text="VAE CPU Offload (saves ~1.3 GB)", variable=self.vae_offload_var)
        self.vae_offload_switch.pack(pady=(0, 15))
    ...
    def _generation_worker(self):
        try:
            ...
            t5_cpu = self.t5_cpu_var.get()
            vae_cpu_offload = self.vae_offload_var.get()

            if (not self.pipeline
                    or self.pipeline.vram_mode != vram_mode
                    or self.pipeline.t5_cpu != t5_cpu
                    or self.pipeline.vae_cpu_offload != vae_cpu_offload):
